                ...
                <thead>
                  <tr>
                    {['Tipo di richiesta', 'data', 'Periodo', 'stato', 'richiedente'].map((column) => (
                      <th key={column} scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 cursor-pointer" onClick={() => handleSort(column)}>
                        {column.charAt(0).toUpperCase() + column.slice(1)}
                        {sortColumn === column ? (sortDirection === 'asc' ? <ArrowUpIcon className="h-5 w-5 inline ml-2" /> : <ArrowDownIcon className="h-5 w-5 inline ml-2" />) : null}
                        <br />
                        <input
                    ...
                    <tr key={calendar.id_calendar}>
                      ...
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                        <span className={classNames(
                          ...
                          calendar.status === 'In Attesa di Approvazione' ? "bg-yellow-100 text-yellow-800" :
                          calendar.status === 'Approvata' ? "bg-green-100 text-green-800" :
                          calendar.status === 'Rifiutata' ? "bg-red-100 text-red-800" :
                          "bg-gray-100 text-gray-800"
                        )}>
                          {calendar.status}
                        </span>
                      </td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{calendar.ownerUser?.name.slice(0, 2).toUpperCase() + calendar.ownerUser?.surname.slice(0, 2).toUpperCase()}</td>
