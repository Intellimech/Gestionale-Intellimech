...
export default function PurchaseCreateForm() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setCompanies(companyRes.data.value.map((company) => ({
          value: company.id,
          label: company.name
        })));
      } catch (error) {
  ...
  const createPurchaseOrder = async (event) => {
    ...
    jsonObject.azienda = selectedCompany?.label; // Use label for company name
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/purchase/create`, jsonObject, { headers: { authorization: `Bearer ${token}` } });
